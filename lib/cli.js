...
var types = {
  ...
  globalDependencies: function *(dir, pkg) {
    ...
    return yield spawn('npm', [ 'install', '-g' ].concat(dependencies), { cwd: dir, stdio: 'inherit', cache: path.resolve('/tmp/.npm', Math.random().toString()) });
  }
...
var actions = {
  ...
  install: function *(dir, pkg) {
    ...
    return yield spawn('npm', [ 'install' ], { cwd: dir, stdio: 'inherit', cache: path.resolve('/tmp/.npm', Math.random().toString()) });
  },
  ...
  this.install = program
    ...
    .action(co(function *(mod) {
      if (!_.isEmpty(mod)) {
        ...
        yield spawn('npm', [ 'install', mod ], { cwd: cwd, stdio: 'inherit', cache: path.resolve('/tmp/.npm', Math.random().toString()) });
        n(nodeVersion);
      }
      else {
