...
var types = {
  ...
  globalDependencies: function *(dir, pkg) {
    ...
    yield spawn('npm', [ 'install', '-g' ].concat(dependencies), { cwd: dir });
  }
...
var actions = {
  ...
  install: function *(dir, pkg) {
    ...
    yield spawn('npm', [ 'install' ], { cwd: dir });
  },
...
function *doAction (action, key) {
  try {
    ...
    return yield fs.writeFile('nex-debug.log', JSON.stringify(log.record, null, 2));
  }
}
