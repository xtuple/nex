...
var types = {
  ...
  globalDependencies: function (dir, pkg) {
    ...
    proc.execSync('npm install -g' + dependencies.join(' '), { cwd: dir });
  }
...
var actions = {
  ...
  install: function (dir, pkg) {
    ...
    proc.execSync('npm install', { cwd: dir });
  },
...
function doAction (action, key) {
  try {
    return _.map(main[key], function (dir, mod) {
      return onModule(action, key, dir);
    });
  }
  catch (e) {
    ...
    return fs.writeFileSync('nex-debug.log', JSON.stringify(log.record, null, 2));
  }
}

function onModule (action, key, _dir) {
  let dir = path.resolve(cwd, _dir);
  if (!fs.existsSync(path.resolve(dir, './package')) && dir.indexOf('/usr/local/lib') !== -1) {
...
nex.cli = function () {
  ...
  this.install = program
    ...
    .on('globalDependencies', function (value) {
      types.globalDependencies(cwd, main);
    })
    ...
    .action(function (mod) {
      if (!_.isEmpty(mod)) {
        ...
        proc.execSync('npm install ' + mod, { cwd: cwd });
        n(nodeVersion);
      }
      else {
        mkdirp.sync(path.resolve(cwd, 'node_modules'));
        doAction('link', 'linkDependencies');
        doAction('install', 'linkDependencies');
      }
    });
  ...
  this.clean = program
    ...
    .action(function () {
      ...
      doAction('clean', 'linkDependencies');
    });
  ...
  this.npm = program
    ...
    .action(function (cmd) {
      ...
      proc.execSync('npm' + process.argv.slice(2).join(' '), { cwd: cwd, stdio: 'inherit' });
    });
