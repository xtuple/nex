...
var types = {
  ...
  globalDependencies: function *(dir, pkg) {
    ...
    return yield spawn('npm', [ 'install', '-g' ].concat(dependencies), { cwd: dir, stdio: 'inherit' });
  }
...
var actions = {
  ...
  install: function *(dir, pkg) {
    ...
    return yield spawn('npm', [ 'install' ], { cwd: dir, stdio: 'inherit' });
  },
...
function *doAction (action, key) {
  try {
    ...
    let iterator = function *() {
      ...
      for (let i = 0; i < _.keys(block).length; i++) {
        let name = _.keys(block)[i];
        let dir = block[name];

        yield onModule(action, key, dir);
      }
...
function *onModule (action, key, _dir) {
  ...
  if (_.isFunction(types[key])) {
    return yield types[key](dir, pkg);
  }
  else {
    return yield actions[action](dir, pkg);
  }
}
...
nex.cli = function () {
  ...
  this.install = program
    ...
    .action(co(function *(mod, cmd) {
      if (_.isString(mod) && !_.isEmpty(mod)) {
        log.info('install', mod);
        yield spawn('npm', [ 'install', mod ], { cwd: cwd, stdio: 'inherit' });
        n(nodeVersion);
      }
      else if (cmd.globalDependencies) {
        yield types.globalDependencies(cwd, main);
      }
      else {
        mkdirp.sync(path.resolve(cwd, 'node_modules'));
        yield doAction('link', 'linkDependencies');
        yield doAction('install', 'linkDependencies');
      }
    }));
  ...
  this.clean = program
    ...
    .action(co(function *() {
      ...
      yield doAction('clean', 'linkDependencies');
    }));
